...
if(opt$model_comp == T){
  ...
  for(group in unique(predictors_list_new$group)){
    for(outer_val in 1:opt$n_outer_fold){
     ...
     # Save outcomes and predictors for subset
		saveRDS(Outcome_Predictors_train, paste0(opt$out, outer_val, '.outcome_pred.rds'))
    saveRDS(Outcome_Predictors_test, paste0(opt$out, outer_val, '.outcome_pred_test.rds'))
    
    # Calculate class weights
    # Calculate total number of cases and controls
    # total_count <- sum(Outcome_Predictors_train_y == "CASE") + sum(Outcome_Predictors_train_y == "CONTROL")

    # Calculate weights by dividing by the total number of cases and controls
    case_weight <- sum(Outcome_Predictors_train_y == "CONTROL") / sum(Outcome_Predictors_train_y == "CASE")
    # control_weight <- sum(Outcome_Predictors_train_y == "CASE") / total_count

    # Assign class weights based on case or control
    class_weights <- ifelse(Outcome_Predictors_train_y == "CASE", case_weight, 1)

  		# If there is only one predictor, use glm
  		if(dim(Outcome_Predictors_train_x_group)[2] > 1){
  				model<- train(y=Outcome_Predictors_train_y, x=Outcome_Predictors_train_x_group, trControl=trainControl(method="cv", seeds=seeds, number=opt$n_inner_fold, classProbs=T, savePredictions = 'final'), method="glmnet", family=opt$family, weights = class_weights)
  		} else {
  				model<- train(y=Outcome_Predictors_train_y, x=cbind(0,Outcome_Predictors_train_x_group), trControl=trainControl(method="cv", seeds=seeds, number=opt$n_inner_fold, classProbs=T, savePredictions = 'final'), method="glm", family=opt$family, weights = class_weights)
  		}
			################
	# Save the model for use in external samples
	################                                

	saveRDS(model$finalModel, paste0(opt$out, outer_val, '.final_model.rds'))
    ...
    saveRDS(indep_pred[[group_name]],paste0(opt$out,'.indep_pred.rds'))
    
    Indep_mod<-summary(lm(scale(as.numeric(indep_pred[[group_name]]$obs)) ~ scale(indep_pred[[group_name]]$pred)))
    
    Indep_log_mod<-glm(indep_pred[[group_name]]$obs ~ scale(indep_pred[[group_name]]$pred),family=opt$family)
    Indep_log<-summary(Indep_log_mod)
    Indep_cis<-exp(confint.default(Indep_log_mod))
    
    Indep_auc <- pROC::auc(roc(indep_pred[[group_name]]$obs, indep_pred[[group_name]]$pred))
    
    Indep_auc1<-roc(indep_pred[[group_name]]$obs, indep_pred[[group_name]]$pred)
    saveRDS(Indep_auc1,paste0(opt$out,'.Indep_auc.rds'))
    ...
    if(opt$family=='binomial'){
      Prediction_summary_all<-rbind(Prediction_summary_all, data.frame(	Model=paste0(group_name,'_group'),
                                                                        ...
                                                                        OR=exp(coef(Indep_log)[2,1]),
																			                                  LowCI=Indep_cis[2,1],
																			                                  HighCI=Indep_cis[2,2],
                                                                        AUC=Indep_auc,
                                                                        R2l=h2l_R2(opt$outcome_pop_prev, coef(Indep_mod)[2,1]^2, sum(indep_pred[[group_name]]$obs == 'CASE')/length(indep_pred[[group_name]]$obs)),
                                                                        N=dim(indep_pred[[group_name]])[1],
                                                                        Ncase=sum(indep_pred[[group_name]]$obs == 'CASE'),
                                                                        Ncont=sum(indep_pred[[group_name]]$obs == 'CONTROL')))
    } else {
      Prediction_summary_all<-rbind(Prediction_summary_all, data.frame(	Model=paste0(group_name,'_group'),
                                                                        R=coef(Indep_mod)[2,1],
                                                                        SE=coef(Indep_mod)[2,2],
